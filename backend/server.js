...
app.get('/api/repos', async (req, res) => {
  try {
    const [rows] = await pool.query(
      'SELECT id, name, provider, url, auth_type, auth_token, auth_username, default_branch, is_local, created_at FROM repositories ORDER BY created_at DESC'
    );
    ...
    res.status(500).json({ detail: 'Failed to fetch repositories' });
  }
});
...
app.delete('/api/repos/:repoId', async (req, res) => {
  try {
    const [result] = await pool.query('DELETE FROM repositories WHERE id = ?', [req.params.repoId]);
...
app.get('/api/operations', async (req, res) => {
  try {
    ...
    res.status(500).json({ detail: 'Failed to fetch operations' });
  }
});

async function createOperation(repoId, operationType, branchName, status, message) {
  const id = uuidv4();
  await pool.query(
    'INSERT INTO operations (id, repo_id, operation_type, branch_name, status, message) VALUES (?, ?, ?, ?, ?, ?)',
    [id, repoId, operationType, branchName, status, message]
  );
  return id;
}

async function updateOperation(id, status, message) {
  await pool.query('UPDATE operations SET status = ?, message = ? WHERE id = ?', [status, message, id]);
}
...
app.post('/api/repos/:repoId/create-branch', async (req, res) => {
  ...
  try {
    const [repos] = await pool.query('SELECT * FROM repositories WHERE id = ?', [repoId]);
    if (repos.length === 0) {
      return res.status(404).json({ detail: 'Repository not found' });
    }
    
    const repo = repos[0];
    const opId = await createOperation(repoId, 'create_branch', branch_name, 'pending', `Creating branch ${branch_name}`);
    ...
    try {
      ...
      const baseBranchName = base_branch || repo.default_branch || 'main';
      ...
      await updateOperation(opId, 'failed', error.message);
      res.status(400).json({ detail: error.message });
    }
  } catch (error) {
    console.error('Error creating branch:', error);
    res.status(500).json({ detail: 'Branch creation failed' });
  }
});
...
app.post('/api/repos/:repoId/push', async (req, res) => {
  ...
  try {
    ...
    const repo = repos[0];
    const opId = await createOperation(repoId, 'push', branch_name, 'pending', `Pushing to ${branch_name}`);
    
    try {
      if (repo.is_local) {
        const git = simpleGit('/app');
        await git.add('.');
        await git.commit(commit_message || 'Auto commit');
    ...
    res.status(500).json({ detail: 'Push failed' });
  }
});
...
app.post('/api/repos/:repoId/merge', async (req, res) => {
  ...
  try {
    ...
    const repo = repos[0];
    const opId = await createOperation(repoId, 'merge', `${source_branch} -> ${target_branch}`, 'pending', `Merging ${source_branch} into ${target_branch}`);
    ...
    res.status(500).json({ detail: 'Merge failed' });
  }
});
...
app.get('/api/deployment-configs', async (req, res) => {
  try {
    ...
    res.status(500).json({ detail: 'Failed to fetch deployment configs' });
  }
});
...
app.delete('/api/deployment-configs/:configId', async (req, res) => {
  try {
    const [result] = await pool.query('DELETE FROM deployment_configs WHERE id = ?', [req.params.configId]);
    if (result.affectedRows === 0) {
      return res.status(404).json({ detail: 'Deployment config not found' });
    }
...
app.post('/api/repos/:repoId/preview-deploy', async (req, res) => {
  ...
  try {
    ...
    try {
      ...
      const directories = [...new Set(files.map(f => path.dirname(f.relativePath)).filter(d => d !== '.'))];
    ...
    res.status(500).json({ detail: 'Failed to generate deployment preview' });
  }
});
...
app.post('/api/repos/:repoId/deploy', async (req, res) => {
  ...
  try {
    ...
    const repo = repos[0];
    const deployConfig = configs[0];
    const opId = await createOperation(repoId, 'deploy', branch_name, 'pending', `Deploying to ${deployConfig.deploy_type}`);
    ...
    res.status(500).json({ detail: 'Deployment failed' });
  }
});
...
app.listen(PORT, '0.0.0.0', async () => {
  console.log(`🚀 DeployFlow API running on http://0.0.0.0:${PORT}`);
